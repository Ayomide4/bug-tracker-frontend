    ...
    case ActionType.RefreshToken:
      if (state.isSignIn && state.auth && state.refresh) {
        return __assign(__assign({}, state), { auth: {
          ...
          expiresAt: action.payload.newAuthTokenExpireIn ? new Date(new Date().getTime() + action.payload.newAuthTokenExpireIn * 60 * 1e3) : state.auth.expiresAt
        }, refresh: {
          token: action.payload.newRefreshToken ? action.payload.newRefreshToken : state.refresh.token,
          expiresAt: action.payload.newRefreshTokenExpiresIn ? new Date(new Date().getTime() + action.payload.newRefreshTokenExpiresIn * 60 * 1e3) : state.refresh.expiresAt
...
var AuthKitError = function(_super) {
...
function useSignIn() {
  ...
  return function(signInConfig) {
    ...
    var expTime = new Date(new Date().getTime() + expiresIn * 60 * 1e3);
    if (context.authState.isUsingRefreshToken) {
      if (!!refreshToken && !!refreshTokenExpireIn) {
        var refreshTokenExpireAt = new Date(new Date().getTime() + refreshTokenExpireIn * 60 * 1e3);
